...
Use `python -m py_compile manage.py modal_newbie_train.py` as the minimum syntax check before committing Python changes.
...
Do not commit `.env`, datasets, model files, logs, outputs, or downloaded reference repositories. Treat Modal credentials and Volume contents as private. Confirm destructive Volume operations in `manage.py` remain guarded by explicit user confirmation.
